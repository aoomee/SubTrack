...
export function RecentlyPaid({ subscriptions, className }: RecentlyPaidProps) {
  ...
  return (
    <Card className={cn("min-h-[280px] flex flex-col", className)}>
      <CardHeader className="flex-shrink-0">
        <CardTitle className="text-base">{t('recentlyPaid')}</CardTitle>
        <CardDescription>
        ...
        {subscriptions.length === 0 ? (
          ...
          <div className="flex-1 divide-y">
            {subscriptions.map((subscription) => (
              <div
                key={subscription.id}
                className="grid min-h-16 grid-cols-[minmax(0,1fr)_auto] items-center gap-4 py-3 first:pt-0 last:pb-0"
              >
